...
4 tienen su follow-up en MP-4 de la guía del miércoles; para los demás, MP-4 dice qué hacer. Los que pasan no llevan ninguno.
...
con `claude-sonnet-5`, el ensayo de esta guía con `gpt-5-mini` en Mac y en Windows, y el recorrido del 13-sep en una
laptop Windows 11.
...
| Ensayo, Mac | `// marcar dueDate vencida (ayer)` encima de `vencida.setAssigneeId(1L);` | la línea asigna responsable, no fecha, y la variable se reemplaza en la siguiente: código muerto |
| Ensayo, Windows | `// Crear dos tareas vencidas: una con dueDate más antigua (debe salir primera)` en el slice | «debe salir primera» porque así la puso el mock, no porque el controlador ordene |
...
```powershell
& $HOME\academyCDMX_Q3\copilot\dia-2\mutar-orden.ps1 -Metodo vencidas | Tee-Object -Append evidencia\dia2\checklist-overdue.txt
```
...
|---|---|---|
| Ensayo, Mac, versión del agente | `Tests run: 8, Failures: 0` y `BUILD SUCCESS` | `vencidas_devuelveSoloVencidasYOrdenadas` tenía **una** tarea vencida: con un elemento, cualquier orden es correcto |
| Ensayo, Mac, después del follow-up | `expected: <5> but was: <1>` y `BUILD FAILURE` | cinco tareas, dos vencidas, entregadas en orden inverso |
| Ensayo, Windows, versión del agente | `Tests run: 8, Failures: 1` y `BUILD FAILURE` | el agente sí entregó las vencidas desordenadas |
...
claude-sonnet-5 el 12-sep, el ensayo en Windows (`getOverdue_retorna200YListaOrdenada`) y el recorrido del 13-sep
