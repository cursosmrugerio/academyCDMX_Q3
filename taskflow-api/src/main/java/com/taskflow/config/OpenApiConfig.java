 ...
 * partir de la petición que sirvió el documento. Detrás de un proxy que termina TLS eso produce
 * «http://<servidor-de-origen>»: el esquema y el puerto del ORIGEN, no los que ve el navegador.
 * Consecuencia MEDIDA: el Swagger se abre por HTTPS pero cada «Try it out» dispara a HTTP, el
 * navegador lo bloquea por mixed content, y los botones quedan de adorno. Con url = "/" la
 * dirección se resuelve contra la página que sirve el Swagger, así que funciona sin tocar nada
 * donde viva la API: el localhost:8080 de cada alumno o detrás de un proxy HTTPS.
 *
